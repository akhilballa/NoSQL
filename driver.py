...
# # #         line = raw_line.strip().replace(" ", "")  # Remove unnecessary spaces
# # #         if not line:
# # #             continue

# # #         # MERGE detection
# # #         merge_match = re.match(r"(SQL|MONGO)\.MERGE\((SQL|MONGO)\)", line)
# # #         if merge_match:
...
# # #         # GET/SET detection
# # #         match = re.match(r"\d+,(SQL|MONGO)\.(GET|SET)\((.*)\)", line)  # Ignore starting number
# # #         if not match:
# # #             print(f" Skipping invalid line: {raw_line}")
# # #             continue

# # #         system, op_type, args = match.groups()  # Only system, op_type, args — not the first number
...
# # #     for _, op in sorted(operations.items(), reverse=True):
# # #         match = re.match(r"SET\(\(\s*(SID\d+)\s*,\s*(CSE\d+)\s*\)\s*,\s*([A-F])\)", op)
# # #         if match:
...
# # #     print(f"✅ MERGE: Applied {oplog2} into {oplog1}, updated {db1.upper()}")
...
# #         # GET/SET detection
# #         match = re.match(r"\d+,(SQL|MONGO|HIVE)\.(GET|SET)\((.*)\)", line)  # Ignore starting number
# #         if not match:
# #             print(f" Skipping invalid line: {raw_line}")
# #             continue
...
# #     for (sid, cid), (timestamp, grade) in latest_set_operations.items():
# #         if db1 == "sql":
# #             sql_set(sid, cid, grade)
# #         elif db1 == "mongo":
# #             mongo_set(sid, cid, grade)
